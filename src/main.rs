...
fn main() {
    ...
    app.add_systems(Startup, (spawn_camera, spawn_payer, spawn_ball,spawn_net));
...
fn spawn_payer(mut commands: Commands) {
...
fn spawn_net(mut commands: Commands) {
    commands.spawn((SpriteBundle {
        transform: Transform::from_translation(Vec3::new(0., 0., 1.)),
        sprite: Sprite {
            color: Color::WHITE,
            custom_size: Some(Vec2::new(10., 250.)),
            ..Default::default()
        },
        ..Default::default()
    },));
...
fn ball_collide(
    ...
    for (ball, mut velocity) in &mut balls {
        ...
        for paddle in &paddles {
            ...
            {
                velocity.0 *= -1.;
                velocity.0.y = rand::thread_rng().gen::<f32>() * 300.;
            }
        }
    }
}
